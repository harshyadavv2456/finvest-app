...
**D4 (option-chain reconstruction) - next up, unblocked now.** Plan (not yet built): filter the R2 instrument master for NFO options where `name == symbol` (NIFTY/BANKNIFTY), batch `getMarketData("FULL", {"NFO": [tokens]})` for LTP/OI/volume/change (~50 tokens/request, chunked) plus one `optionGreek` call per expiry for IV/Greeks, assembled into rows matching `stratax/csv_data_provider.py`'s existing schema (`symbol`, `strikePrice`, `optionType`, `lastPrice`, `openInterest`, `impliedVolatility`, etc. - read and confirmed exact field names from that file) so the frontend needs zero changes.
