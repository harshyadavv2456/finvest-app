...
**Real problem hit, root-caused, and fixed:** the instrument master (needed to resolve a trading symbol to the numeric token AngelOne's API requires) is a ~37MB file. Downloading it live, inside an API request, is unreliable - confirmed via repeated `IncompleteRead` failures on **three** different networks (dev machine, Render, GitHub Actions), always at a random byte offset - a genuine issue with Angel One's own file server (an AWS ELB), not a client-network problem. Root cause found: the server supports HTTP Range requests (`curl -r 0-1023` → 206 Partial Content), so `refresh_angelone_instruments.py` now resumes from the last successfully-received byte each retry instead of restarting from zero - each connection only needs to survive ~1MB before dying, not the whole 37MB. Two bugs found and fixed in this retry logic itself before it worked: (1) the "no progress" abort counter didn't credit bytes received before a mid-stream exception, aborting after 5 attempts despite real progress happening; (2) the job's own 20-minute timeout was too tight for how slow (not broken - genuinely just slow, ~1MB/90s) this download is, cancelling a run that was 37% through and still climbing steadily. Both fixed; **confirmed complete and working end-to-end**: local run finished after 16 attempts (~17min), 157,759 raw instruments downloaded, filtered to 25,241 (NSE/BSE equities + NIFTY/BANKNIFTY F&O), uploaded to R2.

**D1+D2+D3 fully verified live as of this note** - `curl https://finvest-api-gwkz.onrender.com/api/angelone/verify-live` returns `{"ok": true, "step": "ltp", "symbol": "RELIANCE-EQ", "ltp": 1311.5}` - a real live price, auth → instrument lookup → LTP read, all working in production. The GitHub Actions version of the same download (`angelone-instruments.yml`, timeout raised 20→90min) was still running as of this note as a secondary confirmation the fix holds on that infra too - not required for the pipeline to work, since R2 is already populated from the local run.

**D4 (option-chain reconstruction) - shipped and verified live, including in the actual browser.** New `angelone_option_chain.py`, wired into `/api/stratax/option-chain` and `/api/stratax/expiries` for NIFTY/BANKNIFTY (AngelOne first, CSV fallback). Two real bugs found via live testing before it worked: `optionGreek`'s response has no trading-symbol field to join on (identifies rows by `(strikePrice, optionType)` instead - confirmed by inspecting a live response directly, not assumed); the underlying spot price fetch was originally left as a stub, fixed via the well-known NSE index tokens for NIFTY/BANKNIFTY. **Verified in the actual browser** at finvest.fintaxlife.com/stratax: Spot 24247.05, Expiry "25 Aug 2026" (not the old stale Dec 2025), Strikes: 121 - the exact "StrataX is shitty" complaint from earlier in this session is the stale-data problem, and this is the real fix for it, not a CSS pass.

**D5 (visibility half) - shipped and verified live.** `/api/stratax/data-status` now reports `angelone_configured`, `angelone_session_active`, and `last_source_used` (real request tracking, not just config presence) instead of only ever reporting CSV. Verified live: before any option-chain request in a fresh process, `active_source: "csv"` / `angelone_session_active: false`; after one real request, `active_source: "angelone"` / `angelone_session_active: true` / `last_source_used.source: "angelone"`. **D5's retirement half deliberately not done** - `nse_fetcher.py` (the actual NSE-direct-scrape module, invoked by `scripts/fetch_stratax_data.py`) left in place. Not actively harmful to leave (CSV is an explicit fallback, not a second competing primary, so the audit's original "duplicate scripts" concern doesn't apply the same way) - deletion sequenced after AngelOne is confirmed stable across a real trading session, not after one successful test.

E (new India FinDash) - not started, explicitly sequenced after D per the user's own priority order; E1's feature audit is done (see below), E2/E3 are not.
