...
**Not yet done:** D5 (retiring the old NSE-scrape path, updating `/api/stratax/data-status`) - sequenced after D4 produces real data to switch to. E (new India FinDash) - not started, explicitly sequenced after D per the user's own priority order; E1's feature audit is done (see below), E2/E3 are not.
